...
class Solution {
    public List<List<Integer>> allPathsSourceTarget(int[][] graph) {
        ...
        list.add(0);
        dfs(0,graph.length-1,res,list,graph);
        return res;
    }
    public void dfs(int id,int target,List<List<Integer>>res,List<Integer>list,int[][] graph){
        if(id==target){
        ...
        for(int i=0;i<graph[id].length;i++){
            list.add(graph[id][i]);
            dfs(graph[id][i],target,res,list,graph);
...
Question: [LeetCode](https://leetcode.com/problems/bP4bmD/description/)  /  [LeetCode中国](https://leetcode-cn.com/problems/bP4bmD/description/)

Solution: [LeetCode](https://leetcode.com/articles/bP4bmD/)  /  [LeetCode中国](https://leetcode-cn.com/articles/bP4bmD/)
